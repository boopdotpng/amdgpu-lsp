...
mod tests {
  ...
  #[test]
  fn test_split_encoding_variant() {
    let test_cases = vec![
      ("v_add_f32", "v_add_f32", EncodingVariant::Native),
      ("v_add_f32_e32", "v_add_f32", EncodingVariant::E32),
      ("v_add_f32_e64", "v_add_f32", EncodingVariant::E64),
      ("v_add_f32_dpp", "v_add_f32", EncodingVariant::Dpp),
      ("v_add_f32_sdwa", "v_add_f32", EncodingVariant::Sdwa),
      ("v_add_f32_e64_dpp", "v_add_f32", EncodingVariant::E64Dpp),
      ("V_ADD_F32_E64", "V_ADD_F32", EncodingVariant::E64),
      ("s_add_u32", "s_add_u32", EncodingVariant::Native),
    ];

    for (input, expected_base, expected_variant) in test_cases {
      let result = split_encoding_variant(input);
      assert_eq!(
        result.base, expected_base,
        "For input '{}': expected base '{}', got '{}'",
        input, expected_base, result.base
      );
      assert_eq!(
        result.variant, expected_variant,
        "For input '{}': expected variant {:?}, got {:?}",
        input, expected_variant, result.variant
      );
    }
  }

  #[test]
  fn test_encoding_variant_labels() {
    assert_eq!(EncodingVariant::Native.label(), None);
    assert_eq!(EncodingVariant::E32.label(), Some("VOP1/VOP2/VOPC (32-bit)"));
    assert_eq!(EncodingVariant::E64.label(), Some("VOP3 (64-bit)"));
    assert_eq!(EncodingVariant::Dpp.label(), Some("DPP"));
    assert_eq!(EncodingVariant::Sdwa.label(), Some("SDWA"));
    assert_eq!(EncodingVariant::E64Dpp.label(), Some("VOP3 DPP"));
  }

  #[test]
  fn test_find_matching_encoding() {
    // VOP2 instruction encodings (like V_ADD_F32)
    let vop2_encodings = vec![
      "ENC_VOP2".to_string(),
      "ENC_VOP3".to_string(),
      "VOP2_INST_LITERAL".to_string(),
      "VOP2_VOP_DPP16".to_string(),
      "VOP2_VOP_DPP8".to_string(),
      "VOP2_VOP_SDWA".to_string(),
      "VOP3_VOP_DPP16".to_string(),
    ];

    // Test E32 variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::E32),
      Some("ENC_VOP2".to_string())
    );

    // Test E64 variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::E64),
      Some("ENC_VOP3".to_string())
    );

    // Test DPP variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::Dpp),
      Some("VOP2_VOP_DPP16".to_string())
    );

    // Test SDWA variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::Sdwa),
      Some("VOP2_VOP_SDWA".to_string())
    );

    // Test E64_DPP variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::E64Dpp),
      Some("VOP3_VOP_DPP16".to_string())
    );

    // Test Native variant
    assert_eq!(
      find_matching_encoding(&vop2_encodings, &EncodingVariant::Native),
      Some("ENC_VOP2".to_string())
    );
  }

  #[test]
  fn test_get_encoding_description() {
    // Test standard encodings
    assert!(get_encoding_description("ENC_VOP2").is_some());
    assert!(get_encoding_description("ENC_VOP3").is_some());

    // Test DPP encodings
    assert!(get_encoding_description("VOP2_VOP_DPP16").is_some());
    assert!(get_encoding_description("VOP2_VOP_DPP8").is_some());

    // Test SDWA encodings
    assert!(get_encoding_description("VOP2_VOP_SDWA").is_some());

    // Test unknown encoding
    assert_eq!(get_encoding_description("UNKNOWN_ENCODING"), None);
  }
}
